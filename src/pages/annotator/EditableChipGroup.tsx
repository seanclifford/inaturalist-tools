...
export function EditableChipGroup({
	...
	const availableControlledTermValues = controlledTerm.values.filter((x) =>
		othersAnnotations.every((y) => y.controlled_value_id !== x.id),
	);
	...
	if (annotationFunctions) {
		...
		const smartSaveAnnotation = async (params: SaveAnnotationParams) => {
			const existingAnnotation = yourAnnotations.find(
				(a) =>
					a.controlled_attribute_id === controlledTerm.id &&
					a.controlled_value_id !== params.controlledValueId,
			);
		...
		customAnnotationFunctions = {
			saveAnnotation: smartSaveAnnotation,
			deleteAnnotation: smartDeleteAnnotation,
		};
	}

	return (
		<Chip.Group multiple={controlledTerm.multivalued} value={yourSelectedIds}>
			<Group gap="xs">
